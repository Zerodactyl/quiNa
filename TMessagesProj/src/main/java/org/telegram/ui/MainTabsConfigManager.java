...
public class MainTabsConfigManager {
    ...
    public static ArrayList<TabState> getEnabledTabs() {
        ...
        for (TabState state : all) {
            if (state.enabled) enabled.add(new TabState(state));
        }
        if (enabled.isEmpty()) enabled.add(new TabState(TabType.CHATS, true));
        return enabled;
    }
    ...
    public static ArrayList<TabState> copyTabs(List<TabState> tabs) {
        ArrayList<TabState> result = new ArrayList<>();
        for (TabState state : tabs) result.add(new TabState(state));
        return result;
    }
    ...
    public static void saveTabs(List<TabState> tabs) {
        ...
        for (int i = 0; i < copy.size(); i++) {
            ...
            if (!state.enabled) order.append('!');
            order.append(state.type.name());
            if (i != copy.size() - 1) order.append(',');
        }
    ...
    public static int findTabIndex(List<TabState> tabs, TabType type) {
        for (int i = 0; i < tabs.size(); i++) if (tabs.get(i).type == type) return i;
        return -1;
    }
    ...
    public static boolean isTabEnabled(TabType type) {
        for (TabState state : getAllTabs()) if (state.type == type) return state.enabled;
        return false;
    }

    @NonNull
    public static GlassTabView createTabView(Context context, Theme.ResourcesProvider resourceProvider, int currentAccount, TabType type, boolean fromSettings) {
        GlassTabView tabView = switch (type) {
            case CHATS -> GlassTabView.createMainTab(context, resourceProvider, GlassTabView.TabAnimation.CHATS, R.string.MainTabsChats);
            case CONTACTS -> GlassTabView.createMainTab(context, resourceProvider, GlassTabView.TabAnimation.CONTACTS, R.string.MainTabsContacts);
            case SETTINGS -> GlassTabView.createMainTab(context, resourceProvider, GlassTabView.TabAnimation.SETTINGS, R.string.Settings);
            case CALLS -> GlassTabView.createMainTab(context, resourceProvider, GlassTabView.TabAnimation.CALLS, R.string.MainTabsCalls);
            case PROFILE -> GlassTabView.createAvatar(context, resourceProvider, currentAccount, R.string.MainTabsProfile);
        };
    ...
    private static boolean isTabEnabled(List<TabState> tabs, TabType type) {
        for (TabState state : tabs) if (state.type == type) return state.enabled;
        return false;
    }

    private static void setTabEnabled(List<TabState> tabs, TabType type, boolean enabled) {
        for (TabState state : tabs) if (state.type == type) { state.enabled = enabled; return; }
    }

    private static void ensureAtLeastOneEnabled(List<TabState> tabs) {
        for (TabState state : tabs) if (state.enabled) return;
        for (TabState state : tabs) if (state.type == TabType.CHATS) { state.enabled = true; return; }
        if (!tabs.isEmpty()) tabs.get(0).enabled = true;
        else tabs.add(new TabState(TabType.CHATS, true));
    }

    private static boolean getDefaultEnabled(TabType type) { return type != TabType.CALLS; }

    private static void addTabState(ArrayList<TabState> result, EnumSet<TabType> missing, TabType type, boolean enabled) {
        if (!missing.contains(type)) return;
