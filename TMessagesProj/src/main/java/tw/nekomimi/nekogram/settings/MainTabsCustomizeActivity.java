...
public class MainTabsCustomizeActivity extends BaseNekoXSettingsActivity {
    ...
    protected void onCustomCellClick(View view, int position, float x, float y) {
        ...
        if (row == showTabTitlesRow) {
            ...
            NaConfig.INSTANCE.getMainTabsShowTitles().setConfigBool(checked);
    ...
    private void showBottomBarDisplayModeDialog() {
        ...
        final int[] modes = new int[]{ MainTabsHelper.BOTTOM_BAR_MODE_SHOW, MainTabsHelper.BOTTOM_BAR_MODE_HIDE, MainTabsHelper.BOTTOM_BAR_MODE_FLOATING };
            ...
            cell.setOnClickListener(v -> {
                if (MainTabsHelper.getBottomBarDisplayMode() != mode) {
                    ...
                    if (listAdapter != null) {
                        ...
                        if (rowIndex >= 0) listAdapter.notifyItemChanged(rowIndex);
                    }
                }
    ...
    private String getBottomBarDisplayModeName(int mode) {
        if (mode == MainTabsHelper.BOTTOM_BAR_MODE_HIDE) return getString(R.string.MainTabsDisplayModeHide);
        else if (mode == MainTabsHelper.BOTTOM_BAR_MODE_FLOATING) return getString(R.string.MainTabsDisplayModeFloating);
    ...
    private class ListAdapter extends BaseListAdapter {
        public ListAdapter(Context context) { super(context); }
        @Override
    ...
    private class PreviewContainer extends FrameLayout {
        ...
        public void setPreviewCell(MainTabsPreviewCell previewCell) { this.previewCell = previewCell; }
        @Override
        protected void onDraw(Canvas canvas) {
            ...
            if (previewCell != null && previewCell.getMeasuredWidth() > 0 && previewCell.getMeasuredHeight() > 0) {
                rect.set(previewCell.getLeft() + stroke, previewCell.getTop() + stroke, previewCell.getRight() - stroke, previewCell.getBottom() - stroke);
            } else {
    ...
    private static class MainTabsPreviewCell extends org.telegram.ui.MainTabsLayout {
        ...
        public interface OnTabsChangedListener { void onTabsChanged(ArrayList<MainTabsConfigManager.TabState> tabs); }
        public MainTabsPreviewCell(Context context) { super(context, null); setPadding(dp(12), dp(6), dp(12), dp(6)); }
        public void setOnTabsChangedListener(OnTabsChangedListener listener) { onTabsChangedListener = listener; }
        ...
        public void refreshTabs(Context context) { rebuildTabs(context); }
        @Override
        ...
        private void rebuildTabs(Context context) {
            int horizontalPad = dp(12), verticalPad = dp(6);
            ...
            for (int i = 0; i < tabs.size(); i++) {
                ...
                tabView.setOnClickListener(v -> {
                    ...
                    if (clicked.type == MainTabsConfigManager.TabType.CHATS) return;
                    if (clicked.enabled && getEnabledCount() <= 1) return;
        ...
        private int getEnabledCount() {
            int count = 0;
            for (MainTabsConfigManager.TabState tab : tabs) if (tab.enabled) count++;
            return count;
        }
        ...
        private float getVerticalOffset() { return 0f; }
        private boolean startDrag(int index, View view) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) view.startDragAndDrop(clipData, shadow, null, 0);
            else view.startDrag(clipData, shadow, null, 0);
            return true;
        }
        @Override
        public boolean onDragEvent(DragEvent event) {
            ...
            if (action == DragEvent.ACTION_DRAG_STARTED) {
                if (draggingView != null) draggingView.setVisibility(INVISIBLE);
                return true;
                ...
                if (draggingView != null) { draggingView.setVisibility(VISIBLE); draggingView = null; }
                return true;
            }
            return true;
        }
        private int findDropIndex(float x) {
            ...
            if (count <= 0) return 0;
            for (int i = 0; i < count; i++) if (x < getChildAt(i).getRight()) return i;
            return count - 1;
        }
        private void moveTab(int fromIndex, int toIndex) {
            if (fromIndex < 0 || fromIndex >= tabs.size() || toIndex < 0 || toIndex >= tabs.size() || fromIndex == toIndex) return;
        ...
        private void dispatchTabsChanged() {
            if (onTabsChangedListener != null) onTabsChangedListener.onTabsChanged(MainTabsConfigManager.copyTabs(tabs));
        }
    }
